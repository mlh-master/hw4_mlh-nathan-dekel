...
# For later use in CNNs
CNN_X_train = BaseX_train
CNN_X_test = X_test
CNN_X_val = BaseX_val
...
model_relu.add(Dense(300, input_shape=(32 ** 2,), kernel_initializer="he_normal"))
...
save_dir = "results/"
model_name = "final_weights_ReLU64.h5"
model_path = os.path.join(save_dir, model_name)
model_relu.save(model_path)
...
# --------------------------Implement your code here:-------------------------------------
# Setting up the NN (tanh)
new_a_model = Sequential(name="new_a_model")
new_a_model.add(Dense(300, input_shape=(32 ** 2,), kernel_initializer="he_normal"))
...
if not ("results" in os.listdir()):
    os.mkdir("results")
save_dir = "results/"
model_name = "init_weights_25epochs_tanh.h5"
model_path = os.path.join(save_dir, model_name)
...
axs[0].plot(history_a.history['accuracy'])
axs[0].plot(history_a.history['val_accuracy'])
...
if not ("results" in os.listdir()):
    os.mkdir("results")
save_dir = "results/"
model_name = "init_weights_40epochs_tanh.h5"
model_path = os.path.join(save_dir, model_name)
new_a_model.save(model_path)
print('Saved initialized model at %s ' % model_path)
...
if not ("results" in os.listdir()):
    os.mkdir("results")
save_dir = "results/"
model_name = "init_weights_ReLU32.h5"
model_path = os.path.join(save_dir, model_name)
model_relu.save(model_path)
print('Saved initialized model at %s ' % model_path)
...
axs[1].plot(history.history['loss'])
axs[1].plot(history.history['val_loss'])
axs[1].set_title('ReLU Model (w/ batch 32) Loss')
axs[1].set_ylabel('Loss')
axs[1].set_xlabel('Epoch')
axs[1].legend(['Train', 'Test'], loc='upper right')
...
# Saving the final weights
save_dir = "results/"
model_name = "final_weights_ReLU32.h5"
model_path = os.path.join(save_dir, model_name)
model_relu.save(model_path)
print('Saved trained model at %s ' % model_path)
...
def get_net(input_shape,drop,dropRate,reg,filters):
    ########## Filters input added for the final section ##########
    #Defining the network architecture:
    model = Sequential()
    model.add(Permute((1,2,3),input_shape = input_shape))
    model.add(Conv2D(filters=filters[1], kernel_size=(3,3), padding='same', activation='relu',name='Conv2D_1',kernel_regularizer=regularizers.l2(reg)))
    if drop:
        model.add(Dropout(rate=dropRate))
    model.add(BatchNormalization(axis=1))
    model.add(MaxPooling2D(pool_size=(2, 2)))
    model.add(Conv2D(filters=filters[2], kernel_size=(3,3), padding='same', activation='relu',name='Conv2D_2',kernel_regularizer=regularizers.l2(reg)))
    if drop:
        model.add(Dropout(rate=dropRate))
    model.add(BatchNormalization(axis=1))
    ...
    if drop:
        model.add(Dropout(rate=dropRate))
    model.add(BatchNormalization(axis=1))
    model.add(Conv2D(filters=filters[3], kernel_size=(3,3), padding='same', activation='relu',name='Conv2D_5',kernel_regularizer=regularizers.l2(reg)))
    if drop:
        model.add(Dropout(rate=dropRate))
    model.add(BatchNormalization(axis=1))
    model.add(MaxPooling2D(pool_size=(2, 2)))
...
h = NNet.fit(x=CNN_X_train, y=Y_train, batch_size=batch_size, epochs=epochs, verbose=1, validation_split=0,
             validation_data=(CNN_X_val, Y_val), shuffle=True)
...
results = NNet.evaluate(CNN_X_test, Y_test)
...
# --------------------------Implement your code here:-------------------------------------
# Creating a new filters input with the values halved
filters2 = [32, 64, 64, 128, 128]
NNet = get_net(input_shape, drop, dropRate, reg, filters2)
# Defining the optimizer parameters:
AdamOpt = Adam(lr=learn_rate, decay=decay)
